...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int K = 0;  // 하루에 최소 K원 인출 -
        int maxMoney = 0;
        int minMoney = Integer.MAX_VALUE;
        for(int i = 0; i < N; i++) {
            moneyList.add(Integer.parseInt(in.readLine())); // i 번째 날에 필요한 금액
            maxMoney += moneyList.get(i);
            minMoney = Math.min(minMoney, moneyList.get(i));
        }
        /*
        -> 정확히 K원을 M번 인출해서 매일 필요한 금액을 모두 충당
         */
        int left = minMoney, right = maxMoney;
        while (left <= right) {
            int mid = (left + right) / 2; // 인출 금액
            if(getCount(mid)) { // 인출 금액이 더 큰 경우 -> 더 줄여서 최소 금액 K 찾기 ->  금액 줄이기
                right = mid - 1;
    ...
    public static boolean getCount(int money) { //money(=K)원 씩 몇 번 인출해야 하는지
        ...
        for(int i = 0; i < N; i++) {
            ...
            // 쓰고 남은 돈으로 i 번째 날 돈이 부족하면 그대로 넣고
